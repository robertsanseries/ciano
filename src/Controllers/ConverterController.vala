    ...
    public class ConverterController : Object {
                ...
                while (true) {
                    ...
                    if (this.check_for_errors (line, row)) {
                        if (this.settings.error_notify) {
                            this.send_notification (item, Properties.TEXT_ERROR_IN_CONVERSION);
                        }
                        break;
                    }
                }
            ...
            Idle.add (() => {
                ...
                try {
                    if (proc.wait_check ()) {
                        ...
                        row.status_label.label = Properties.TEXT_SUCESS_IN_CONVERSION;

                        if (this.settings.complete_notify) {
                            this.send_notification (item, Properties.TEXT_SUCESS_IN_CONVERSION);
                        }
                    }
                    ...
                    if (status == 9) {
                        row.status_label.label = Properties.TEXT_CANCEL_IN_CONVERSION;
                    } else {
                        row.status_label.label = Properties.TEXT_ERROR_IN_CONVERSION;
        ...
        private bool check_for_errors (string line, RowConversion row) {
            ...
            if (line.contains ("No such file")) {
                error_msg = Properties.MSG_ERROR_NO_SUCH_FILE_DIRECTORY;
            } else if (line.contains ("Invalid argument")) {
                error_msg = Properties.MSG_ERROR_INVALID_ARGUMENT;
            } else if (line.contains ("Experimental codecs")) {
                error_msg = Properties.MSG_ERROR_CODECS;
            } else if (line.contains ("Invalid data found")) {
                error_msg = Properties.MSG_ERROR_INVALID_INPUT_DATA;
            }
